  ...
  if (isset($_GET['control']) && !empty($_GET['control'])) {
    ...
    <div class="row justify-content-center">
      <div class="col-lg-12 mt-3 mb-3">
          ...
          <form  action="#" method="post" id="dataFormBoys" class="px-3">
            <div class="row">
         ...
         <h3 class="text-muted text-center">Data Form Info</h3><hr>
          <div id="showError"></div><br>

          <div class="row">
            
            <div class="form-group col-md-6">
              <label for="form-control">Name of Boy <sup class="text-danger">*</sup></label>
           ...
           <small class="text-muted">If the boy does not have state ID thick this box</small>
            </div>
            <div class="form-group col-md-6">
            ...
            <div class="form-group col-md-6">
              <input type="submit" name="addBoys" id="addBoysBtn" value="Add Boy" class="btn btn-info btn-block btn-lg">
            </div>
            <div class="col-md-6">
                <button  id="RedirectBoyOnly" class="btn btn-md btn-block btn-danger">
               Click me after adding all the boys!
                </button> 
...
<script type="text/javascript">
  $(document).ready(function(){
    //send feedback ajax
      $('#addBoysBtn').click(function(e){
      if($('#dataFormBoys')[0].checkValidity()){
        e.preventDefault();
          $(this).val('Please wait...');
          $.ajax({
            url: '../data-process.php',
            method: 'post',
            data: $('#dataFormBoys').serialize()+'&action=addInfo',
            success:function(response){
              console.log(response);
              if ($.trim(response) === 'true') {
                $('#dataFormBoys')[0].reset();
                $('#addBoysBtn').val('Add Boy');
                $('#showError').html('<span class="text-success">Boy Added!</span>');
