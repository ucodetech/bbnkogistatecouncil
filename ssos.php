...
@keyframes spin {
  ...
  <div class="container-fluid bg-light text-dark">
    <div class="table-responsive">
      <h3 class="text-center text-info mt-1"><i class="fa fa-spinner"></i>&nbsp;State Secratary Organizers In Kogi State Council&nbsp;<i class="fa fa-spinner"></i></h3><hr class="hrs">
      
      <table class="table table-striped table-hover table-bordered" id="sso">
        <thead>
          <th>#</th>
          <th>Name</th>
          <th>From</th>
        ...
        <tbody>
          <?php 
          $i = 0;
          foreach ($statesso as $sso):
            $date = date('Y');
            if ($sso->served_finish_date > $date) {
              $tillnow = '<span class="text-center text-success">To Date&nbsp;<i class="fa fa-check fa-lg"></i></span>';
            }else{
                $tillnow = pretty_dated($sso->served_finish_date);
            }
            $i = $i + 1;
          ...
          <tr>
            <td><?= $i ?></td>
            <td><?=$sso->sso_name ?></td>
            <td><?=pretty_dated($sso->served_start_date) ?></td>
            <td><?=$tillnow ?></td>
